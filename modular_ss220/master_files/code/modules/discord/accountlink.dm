...
/client/verify_in_discord()
	...
	if(cached_one_time_token && cached_one_time_token != "")
		message = "Вы уже сгенерировали токен - [cached_one_time_token]. В канале дом-бота используйте команду <br/> <span class='warning'>[prefix]привязать_аккаунт [cached_one_time_token]</span>"
		...
		message = "В канале дом-бота используйте команду <br/> <span class='warning'>[prefix]привязать_аккаунт [one_time_token]</span>"
