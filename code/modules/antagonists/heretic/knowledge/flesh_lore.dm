...
/datum/heretic_knowledge/limited_amount/flesh_ghoul
	...
	desc = "Позволяет трансмутировать труп и мак, чтобы создать Безголосого мертвеца. \
		...
		Одновременно можно иметь только двух."
...
/datum/heretic_knowledge/limited_amount/flesh_ghoul/on_finished_recipe(mob/living/user, list/selected_atoms, turf/loc)
	...
	if(!soon_to_be_ghoul.mind || !soon_to_be_ghoul.client)
		...
		var/list/mob/dead/observer/candidates = poll_candidates_for_mob("Вы хотите играть за [soon_to_be_ghoul.real_name], Безголосого мертвеца?", ROLE_HERETIC, ROLE_HERETIC, 5 SECONDS, soon_to_be_ghoul)
...
/datum/heretic_knowledge/spell/flesh_surgery
	...
	desc = "Дает вам Knit Flesh, заклинание, которое позволяет извлекать органы из жертв \
...
/datum/heretic_knowledge/ultimate/flesh_final
	...
	desc = "Ритуал вознесения Пути плоти. \
		...
		Кроме того, вы можете вызывать в три раза больше гулей и Безголосых мертвецов, \
		а также создавать неограниченное количество клинков, чтобы вооружить их всех." // TODO: оружие или руки? надо код чекнуть
...
/datum/heretic_knowledge/ultimate/flesh_final/on_finished_recipe(mob/living/user, list/selected_atoms, turf/loc)
	. = ..()
	priority_announce("[generate_heretic_text()] Вечно закручивающийся вихрь. Реальность раскручивается. С ВЫТЯНУТЫМИ РУКАМИ, ВЛАСТЕЛИН НОЧИ, [user.real_name] возвысился! Бойтесь вечно изгибающейся руки! [generate_heretic_text()]", "[generate_heretic_text()]", ANNOUNCER_SPANOMALIES)
