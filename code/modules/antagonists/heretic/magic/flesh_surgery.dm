...
/datum/action/cooldown/spell/touch/flesh_surgery/proc/heal_organ(obj/item/melee/touch_attack/hand, obj/item/organ/to_heal, mob/living/carbon/caster)
	...
	if(to_heal.damage < organ_hp_to_heal)
		...
		to_heal.balloon_alert(caster, "орган вылечен")
		playsound(to_heal, 'sound/magic/staff_healing.ogg', 30)
		new /obj/effect/temp_visual/cult/sparks(get_turf(to_heal))
		caster.visible_message(
			span_warning("Рука [caster] светится ярким красным светом, \the [to_heal] восстанавливается до хорошего состояния!"),
			span_notice("Ваша рука светится ярким красным светом, \the [to_heal] восстанавливается в хорошее состояние!"),
		)
	else
...
/datum/action/cooldown/spell/touch/flesh_surgery/proc/steal_organ_from_mob(obj/item/melee/touch_attack/hand, mob/living/victim, mob/living/carbon/caster)
	...
	if(carbon_victim == caster)
		...
		caster.visible_message(
			span_danger("Рука [caster] светится ярким светом, когда они тянутся к их [parsed_zone]!"),
			span_userdanger("Ваша рука светится ярким красным светом, когда вы тянетесь прямо к своему собственному [parsed_zone]!"),
		)

	else
		carbon_victim.visible_message(
			span_danger("Рука [caster] светится ярким светом, когда они тянутся к [parsed_zone] [carbon_victim]!"),
	...
	if(carbon_victim == caster)
		caster.visible_message(
			span_bolddanger("[caster] извлекает свой же [chosen_organ] из их [parsed_zone]!!"),
			span_userdanger("Вы извлекаете [chosen_organ] из вашего [parsed_zone]!!"),
		)
